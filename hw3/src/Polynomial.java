...
public class Polynomial {
    ...
    public Polynomial dx() {
        ...
        for (BigInteger index : this.monos.keySet()) {
            HashMap<Polynomial, Monomial> value = this.monos.get(index);
            for (Polynomial p : value.keySet()) {
                poly = poly.addPoly(value.get(p).dx());
            }
        }
        return poly;
    }
    ...
    public BigInteger hasGcd() {
        BigInteger gcd = BigInteger.ZERO;
        for (BigInteger index : this.monos.keySet()) {
            HashMap<Polynomial, Monomial> value = this.monos.get(index);
            for (Polynomial p : value.keySet()) {
                gcd = gcd.gcd(value.get(p).getRatio().abs());
            }
        }
        return gcd;
    }

    public String divGcd(BigInteger gcd) {
        Polynomial poly = new Polynomial();
        for (BigInteger index : this.monos.keySet()) {
            HashMap<Polynomial, Monomial> value = this.monos.get(index);
            for (Polynomial p : value.keySet()) {
                Monomial beDiv = value.get(p);
                Monomial mono = new Monomial(beDiv.getRatio().divide(gcd),
                        beDiv.getIndex(), beDiv.getExp());
                poly.addMono(mono);
            }
        }
