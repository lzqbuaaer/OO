...
public class MyRedEnvelopeMessage implements RedEnvelopeMessage {
    ...
    public boolean equals(Object obj) {
        if (obj instanceof Message) {
            return (((RedEnvelopeMessage) obj).getId() == id);
        }
        return false;
    }
    ...
    public void send0() {
        ...
        this.person1.addMoney(-this.money);
        this.person2.addMoney(this.money);
    }

    public void send1() {
        this.person1.addSocialValue(this.socialValue);
        ((MyTag) this.tag).addSocialValue(socialValue);
        int size = this.tag.getSize();
        if (size != 0) {
            int m = this.money / size;
            this.person1.addMoney(-m * size);
            ((MyTag) this.tag).addMoney(m);
        }
    }
}
