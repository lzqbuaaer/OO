...
public class PathTest {//包含题目[3]
    ...
    public void testExtractLoopPath() {
        ...
        for (int i = 0; i < oldNodes.size(); i++) {
            for (int j = i + 1; j < oldNodes.size(); j++) {
                if (oldPath.getNode(i) == oldPath.getNode(j)) {
                    f1 = true;
                }
            }
        }
        if (f1 && oldPath.isValid()) {
            Path result = path.extractLoopPath();
            assertNotNull(result);
            boolean f2 = false;
            for (int i = 0; i < path.size() - 1; i++) {
                boolean f3 = true;
                for (int j = 0; j < result.size(); j++) {
                    if (!(i + j < path.size() && result.getNode(j) == path.getNode(i + j))) {
                        f3 = false;
                        break;
                    }
                }
            ...
            assertTrue(f2);
            assertEquals(result.getNode(0), result.getNode(result.size() - 1));
        } else {
